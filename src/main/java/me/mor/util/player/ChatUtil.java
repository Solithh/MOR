...
import static me.mor.util.traits.Util.mc;
...
public class ChatUtil {
    ...
    public static void sendClientSideMessage(Component message, String identifier) {
        ...
        chat.mor$removeMessage(signature);
        chat.mor$addMessage(new GuiMessage(mc.gui.getGuiTicks(), message, signature, getMessageTag()));
    }

    public static Component getClientNameComponent() {
        return Component.empty().withColor(MOR.colorManager.getColorAsInt()).append("MOR");
    }

    private static GuiMessageTag getMessageTag() {
        return new GuiMessageTag(MOR.colorManager.getColorAsInt(), null, null, null);
    }
