...
import me.mor.features.gui.Widget;
...
public class Slider
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.rect(context, this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? MOR.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().color.getValue().getAlpha()) : MOR.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().topColor.getValue().getAlpha()));
        drawString(this.getName() + " " + ChatFormatting.GRAY + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf((this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) MORGui.getClickGui().getTextOffset(), -1);
    }
    ...
    public boolean isHovering(int mouseX, int mouseY) {
        for (Widget widget : MORGui.getClickGui().getComponents()) {
