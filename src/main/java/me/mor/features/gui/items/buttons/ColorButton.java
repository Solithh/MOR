...
public class ColorButton extends Button {
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        ...
        drawString(this.getName(), this.x + 2.3f, this.y - 1.7f - (float) MORGui.getClickGui().getTextOffset(), -1);
        ...
        if (open) {
            ...
            RenderUtil.rect(context, pickerX, this.y + yOffset, pickerX + buttonWidth, this.y + yOffset + 14,
                    hoveringCopy ? MOR.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().topColor.getValue().getAlpha()) : 0x11555555);
            ...
            RenderUtil.rect(context, pickerX + buttonWidth + 1, this.y + yOffset, pickerX + buttonWidth * 2 + 1, this.y + yOffset + 14,
                    hoveringPaste ? MOR.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().topColor.getValue().getAlpha()) : 0x11555555);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseButton == 0) {
            ...
            if (hoveringCopy) {
                MORGui.setColorClipboard(setting.getValue());
                mc.getSoundManager().play(SimpleSoundInstance.forUI(SoundEvents.UI_BUTTON_CLICK, 1f));
            }
            if (hoveringPaste && MORGui.getColorClipboard() != null) {
                setting.setValue(MORGui.getColorClipboard());
                hsb = Color.RGBtoHSB(setting.getValue().getRed(), setting.getValue().getGreen(), setting.getValue().getBlue(), null);
                mc.getSoundManager().play(SimpleSoundInstance.forUI(SoundEvents.UI_BUTTON_CLICK, 1f));
            }
        }
